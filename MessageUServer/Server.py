        ...
        try:
            ...
            if data:
                ...
                if code == RequestCode.userRegister.value:
                    ...
                    # add new entry
                    uid = uuid.uuid4()
                    self.__clients.append(ClientEntry.ClientEntry(tuple(uid.bytes), name, pub_key))
                    ...
                    values = (
                        ...
                        (self.__clients.__len__() - 1) * (NAME_MAX_SIZE + CLIENT_ID_LEN))
                    for entry in self.__clients:
                    ...
                    for msg in self.__messages:  # loop over the messages
                        if msg.ToClient == source_cid:  # if this msg destination is for the client who requested to pull
                            ...
                            payload += (msg.FromClient + tuple(struct.pack("I", msg.ID)) + tuple(
                                struct.pack("B", msg.Type)) +
                                        tuple(struct.pack("I", content_len)) + tuple(msg.Content))

                            frmt += (this_msg_size_in_bytes * 'B')

                    self.__messages = [msg for msg in self.__messages if
                                       msg.ToClient != source_cid]  # remove the messages
